...
This program supports the new RIGOL oscilloscopes with "Centaurus" technology, with all functionalities. To use a file generated by one of these devices, enter the `-n` flag so that the program can correctly read the new file format.

For example, the previous command becomes:

       C:\rigol2spice\rigol2spice.exe -n D:\NewFile1.csv D:\my_capture.txt
...
The `--offset` option allows you to apply a vertical offset to a signal. In the argument, use the `M` or `P` prefixes for down (minus) and up (plus) direction then the desired value, e.g.:

* `--offset P1` will offset the signal 1 (vertical unit) up (positive)
* `--offset M0.500` will offset the signal negatively (down) by 500m (vertical unit)
...
## Usage reference
    ...
    OPTIONS:
      -n, --new-models                Adopts the format used by the newer Rigol Centaurus platform oscilloscopes.
      ...
      -o,  --offset <offset>          Offset value for signal (use M and P prefixes)
